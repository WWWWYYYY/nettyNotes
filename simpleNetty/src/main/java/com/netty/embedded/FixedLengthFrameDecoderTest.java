...
public class FixedLengthFrameDecoderTest {
    ...
    public void testFramesDecoded() {
        ...
        //writeInbound返回内容时（即FixedLengthFrameDecoder调用decode方法是否执行了out.add），如果有对象返回则返回true，没有则返回false
        //这里把input一个字节写入到channel中 预期结果为false（即FixedLengthFrameDecoder调用decode方法不会执行out.add）
        ...
        read.release();//没调用一次release让read向下滑动3个字节
